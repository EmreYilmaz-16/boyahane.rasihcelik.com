...
<cfscript>
	...
	{
		session.ep.money = session.pda.money;
		session.ep.money2 = session.pda.money2;
		session.ep.userid = session.pda.userid;
		session.ep.company_id = session.pda.our_company_id;
		session.ep.our_company_id = session.pda.our_company_id;
		session.ep.period_id = session.pda.period_id;
	}
	else if(isdefined("session.ep") and isDefined("session.ep.userid"))
	{
		session.ep.money = session.ep.money;
		session.ep.money2 = session.ep.money2;
		session.ep.userid = session.ep.userid;
		session.ep.company_id = session.ep.company_id;
		session.ep.period_id = session.ep.period_id;
	}
</cfscript>
<cffunction name="specer" returntype="string" output="false">	
	...
	<cfargument name="main_product_money" type="string" default="#session.ep.money#"><!--- spec eklenecek urunun fiyat para birimi --->
	...
	<cfargument name="other_money" type="string" default="#session.ep.money2#">
	...
	<cfargument name="spec_money_select" type="string" default="#session.ep.money2#"><!--- secili olan para birimi --->
...
<!--- <cfdump var="#arguments#">
 ...
 <cfif isDefined('session.ep.userid')>
	<cfif isdefined("fusebox.use_spect_company") and len(fusebox.use_spect_company) and isdefined("fusebox.spect_company_list") and listfind(fusebox.spect_company_list,session.ep.company_id)>
...
<cfelseif isdefined("dsn3")>
	...
	<cfelseif isdefined("arguments.spec_row_count")>		
		<cfset wrk_id = dateformat(now(),'YYYYMMDD')&timeformat(now(),'HHmmss')&'_#session.ep.userid#_#arguments.main_stock_id#_#arguments.spec_row_count#_'&round(rand()*100) />
	<cfelse>
		<cfset wrk_id = dateformat(now(),'YYYYMMDD')&timeformat(now(),'HHmmss')&'_#session.ep.userid#_#arguments.main_stock_id#_'&round(rand()*100) />
	</cfif>
	...
	<cfif arguments.only_main_spec eq 0>
		...
		<cfif len(main_spec_id) and (isdefined('arguments.add_to_main_spec') and arguments.add_to_main_spec) or (listlen(arguments.product_price_list,',') eq 0 and listlen(arguments.product_money_list,',') eq 0)>
			...
			<cfif arguments.is_purchasesales eq 1><!--- satissa sadece buraya girer alis ise sadece standart alis fiyatini almali --->
				<cfif arguments.company_id gt 0>
					<cfquery name="GET_PRICE_CAT_CREDIT" datasource="#arguments.dsn_type#">
						SELECT PRICE_CAT FROM #dsn_alias#.COMPANY_CREDIT WHERE COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.company_id#"> AND OUR_COMPANY_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#session.ep.company_id#">
					</cfquery>
			...
			if(listlen(arguments.product_id_list,',') and listlen(arguments.stock_id_list,','))
			{
				...
				{
					if(GET_PRICE_MAIN_PROD.MONEY eq session.ep.money)
					{
				...
				{//product_id 0 dan fakli ise fiyati alinir degilse 0 atiyoz
					if(listgetat(arguments.product_id_list,s,',') gt 0 and isdefined("attributes.price_catid") and len(attributes.price_catid) and isdefined("cfquery"))
					...
					else
					{
						...
						arguments.product_money_list=listappend(arguments.product_money_list,session.ep.money,',');
					}
				}
			}
			</cfscript>
			...
			<cfscript>
				...
				if(listlen(arguments.money_list,',') eq 0 or listlen(arguments.money_rate1_list,',') or listlen(arguments.money_rate2_list,',') eq 0)
				{
					if(isdefined("session.ep.company_id"))
						GET_MONEY_SPEC = cfquery(SQLString:'SELECT MONEY AS MONEY_TYPE,RATE2,RATE1 FROM #dsn_alias#.SETUP_MONEY SETUP_MONEY WHERE COMPANY_ID=#session.ep.company_id# AND PERIOD_ID=#session.ep.period_id# AND MONEY_STATUS=1',Datasource:arguments.dsn_type,is_select:1);	
					else
						GET_MONEY_SPEC = cfquery(SQLString:'SELECT MONEY AS MONEY_TYPE,RATE2,RATE1 FROM #dsn_alias#.SETUP_MONEY SETUP_MONEY WHERE COMPANY_ID=#session.ep.our_company_id# AND PERIOD_ID=#session.ep.period_id# AND MONEY_STATUS=1',Datasource:arguments.dsn_type,is_select:1);	
			...
			<cfquery name="ADD_VAR_SPECT" datasource="#arguments.dsn_type#">
				...
				INTO
					...
					(
						...
						<cfif len(arguments.main_product_money)>'#arguments.main_product_money#'<cfelse>'#session.ep.money#'</cfif>,
						#arguments.spec_is_tree#,
		...
		<cfelseif isdefined('arguments.spec_id') and len(arguments.spec_id)>
			<cfquery name="UPD_VAR_SPECT" datasource="#arguments.dsn_type#">
				...
				SET
					...
					PRODUCT_AMOUNT_CURRENCY=<cfif len(arguments.main_product_money)>'#arguments.main_product_money#'<cfelse>'#session.ep.money#'</cfif>,
	...
	<cfif arguments.only_main_spec>
		<cfset spec_return='#main_spec_id#,0,#replace(arguments.spec_name,',','','all')#,0,0,#session.ep.money#,0,#session.ep.money#,0,0,#is_new_spec#' />
	<cfelse>
	...
	<cffunction name="add_serial_no" output="false">
           ...
           <cfset letters = "a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,r,s,t,u,v,y,z,1,2,3,4,5,6,7,8,9,0" />
            ...
            <!--- 
                ...
                my_is_sale = 1 (satış)
            --->
            <cfif listfindnocase('59,73,74,75,76,77,80,82,84,86,87,110,115,140',arguments.process_type,',')><!--- Alış ve giriş --->
