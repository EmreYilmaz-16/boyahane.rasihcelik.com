...
<cfif 1 eq 1><cfset wrk_id = dateformat(now(),'YYYYMMDD')&timeformat(now(),'HHmmss')&'_'&round(rand()*100)></cfif><!--- pda den gelen işlemler için --->
...
<cfif get_purchase.recordcount>
	<script type="text/javascript">
		alert("Girdiğiniz İrsaliye Numarası Seçilen Cari Adına Kullanılmış Lütfen Tekrar Giriniz !");
		window.location.href='<cfoutput>#cgi.referer#</cfoutput>';
		//history.back();
	</script>
	<cfabort>
...
<cflock name="#CreateUUID()#" timeout="20">
	<cftransaction>
		<cfquery name="ADD_PURCHASE" datasource="#DSN2#" result="MAX_ID">
			INSERT INTO
                ...
                (
                    ...
                    <cfif isdefined("attributes.evr_no") and len(attributes.evr_no)>'#attributes.evr_no#'<cfelse>NULL</cfif>,
                    ...
                    <cfif isdefined('attributes.aciklama') and len(attributes.aciklama)>'#attributes.aciklama#'<cfelse>NULL</cfif>,
        ...
        <cfquery name="insertInfoPlus" datasource="#DSN2#">
                INSERT INTO SHIP_INFO_PLUS (
                    SHIP_ID,
                    PROPERTY1,
                    PROPERTY2,
                    PROPERTY3,
                ...
                VALUES (
                    #MAX_ID.IDENTITYCOL#,
                    '#attributes.ucret_dur#',
                    '#attributes.kum_dur#',
                    '#attributes.evr_no_2#',
                    '#attributes.iade_neden#',
                    '#gr_mtul#',
                    '#top_adedi#',
                    '#h_gram#',
                    '#order_no#',
        ...
        <cfset attributes.rows=" ,">
        <!--- from="1" to="#attributes.rows_#"---->
		...
		<cfscript>
			if(get_process_type.is_stock_action eq 1)//Stok hareketi yapılsın
			{
				...
				for(_inde_=1;_inde_ lte 1;_inde_=_inde_+1)
				{
					...
					{	writeOutput("rowkontrolü ve stok haraketi ifine girdim");
						...
						writeOutput("<br>"&sr_amount_other_list)
						if(isdefined('attributes.unit_other#_ind_#') and len(Evaluate('attributes.unit_other#_ind_#')) )
                            ...
                            writeOutput("<br>"&sr_unit_other_list)
						if(isdefined('attributes.deliver_date#_ind_#') and isdate(Evaluate('attributes.deliver_date#_ind_#')) )
                            ...
                            writeOutput("<br>"&sr_deliver_date_list)
					}
				}
				if(len(sr_product_id_list) and len(sr_stock_id_list))
				{
                    
                    new_dsn3 = "catalys_prod_1";
                   // include "add_stock_rows.cfm";
                   // include "testtt.cfm";
			...
			{
				include('add_ship_row_relation.cfm'); 
			...
			{
				include('add_order_row_reserved_stock_pda.cfm'); //siparis irsaliye iliskisini, siparis satır aşama ve rezerve tipini update ediyor
				add_reserve_row(
...
<cfif isdefined('attributes.service_stock_id') and len(attributes.service_stock_id) and isdefined('attributes.service_serial_no') and len(attributes.service_serial_no)> <!--- servis basvuru eklemeden cagrılmıssa --->
	...
	<cfif get_pro_guaranty.recordcount>
		<cfinclude template="add_serial_no.cfm">
