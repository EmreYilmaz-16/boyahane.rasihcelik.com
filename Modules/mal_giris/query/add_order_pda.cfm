<cfparam name="attributes.order_date" default="#now()#">
<cfparam name="attributes.basket_due_value_date_" default="#now()#">
<cfparam  name="attributes.deliverdate" default="">
<cfparam  name="attributes.ship_date" default="">
<cfparam  name="attributes.priority_id" default="1">
...
<cfparam name="FORM.GENEL_INDIRIM" default="0">
<cfparam name="attributes.PROCESS_STAGE" default="83">
<cfset ATTRIBUTES.ORDER_HEAD = ATTRIBUTES.parti_kodu>
<cfset DSN3 = "#DSN#_1">
...
<!--- <cftry> --->
	<cfquery name="ADD_ORDER" datasource="#DSN3#">
		...
		(
			...
			<cfif isdefined("get_customer_info.COMPANY_ADDRESS") and len(get_customer_info.COMPANY_ADDRESS)>'#get_customer_info.COMPANY_ADDRESS#'<cfelse>NULL</cfif>,
			<cfif isdefined("get_customer_info.CITY") and len(get_customer_info.CITY)>#get_customer_info.CITY#<cfelse>NULL</cfif>,
			<cfif isdefined("get_customer_info.COUNTY") and len(get_customer_info.COUNTY)>#get_customer_info.COUNTY#<cfelse>NULL</cfif>,
				'#attributes.order_head#',
			<cfif isdefined('attributes.DESCR') and len(attributes.DESCR)>'#attributes.DESCR#'<cfelse>NULL</cfif>,
	...
	<cfquery name="insPartnerRel" datasource="#dsn#">
		INSERT INTO catalyst_prod_1.ORDER_TO_SHIP_PRT (ORDER_ID,PERIOD_ID,SHIP_ID) VALUES (#GET_MAX_ORDER.MAX_ID#,#attributes.peryodid#,#attributes.sipoid#)
	</cfquery>
	
	<cfinclude  template="add_info_plus.cfm">
	...
	<cfloop from="1" to="#attributes.rows_#" index="i">
		<cfset ix=listGetAt(attributes.STOCK_ID_LIST, i)>
		<cfquery name="getSinfo" datasource="#dsn3#">
			SELECT * FROM STOCKS WHERE STOCK_ID=#ix#
		</cfquery>
		<cfset DSN3_ALIAS ="#dsn#_1">
		<cfset "attributes.STOCK_ID#i#"=getSinfo.STOCK_ID>
		<cfset "attributes.PRODUCT_ID#i#"=getSinfo.PRODUCT_ID>
		<cfset "attributes.PRODUCT#i#"=getSinfo.PRODUCT_ID>
		<cfset "attributes.product_name#i#"=getSinfo.PRODUCT_NAME>
		<cfset "attributes.row_kontrol#i#"=1>
		<cfset "attributes.AMOUNT#i#"=attributes.kg>
		<cfset "attributes.unit#i#"="kg">
		<cfset "attributes.spect_id#i#"="">
		<cfset "attributes.unit_id#i#"=getSinfo.PRODUCT_UNIT_ID>
		<cfif evaluate('attributes.row_kontrol#i#')>
			...
			<cfquery name="ADD_ORDER_ROW" datasource="#DSN3#">
				...
				VALUES
				(
					#get_max_order.max_id#, <!---ok---->
					#evaluate("attributes.product_id#i#")#, <!---ok---->
					#evaluate("attributes.stock_id#i#")#, <!---ok---->
					#evaluate("attributes.amount#i#")#,<!---ok---->
					'#evaluate("attributes.unit#i#")#', <!---ok---->
					#evaluate("attributes.unit_id#i#")#, <!---ok---->
					<cfif isDefined("attributes.price#i#") and len(evaluate("attributes.price#i#"))> 
						#evaluate("attributes.price#i#")#, <!---ok---->
					</cfif>
						#evaluate("attributes.tax#i#")#, <!---ok---->
					<cfif isdefined("attributes.row_nettotal#i#") and len(evaluate("attributes.row_nettotal#i#"))>#evaluate("attributes.row_nettotal#i#")#<cfelse>NULL</cfif>,<!---ok---->
					<cfif isdefined("attributes.duedate#i#") and len(evaluate("attributes.duedate#i#"))>#evaluate("attributes.duedate#i#")#<cfelse>NULL</cfif>,  <!---ok---->
						'#evaluate("attributes.product_name#i#")#', <!---ok---->
					<cfif isdefined("attributes.deliver_date#i#") and isdate(evaluate('attributes.deliver_date#i#'))>#evaluate('attributes.deliver_date#i#')#<cfelse>NULL</cfif>, <!---ok---->
					<cfif isdefined("attributes.deliver_dept#i#") and len(trim(evaluate("attributes.deliver_dept#i#"))) and len(listfirst(evaluate("attributes.deliver_dept#i#"),"-"))> <!---ok---->
						#listfirst(evaluate("attributes.deliver_dept#i#"),"-")#, <!---ok---->
					<cfelse>
						NULL,
					</cfif>
					<cfif isdefined("attributes.deliver_dept#i#") and listlen(trim(evaluate("attributes.deliver_dept#i#")),"-") eq 2 and len(listlast(evaluate("attributes.deliver_dept#i#"),"-"))> <!---ok---->
	...
	<cfscript>
		...
		include('add_order_row_reserved_stock_pda.cfm',''); //rezerve edilen satırlar icin ORDER_ROW_RESERVED'a kayıt atıyor.
		add_reserve_row(
	...
	<!---	<cf_workcube_process
			...
			warning_description='Sipariş : #get_max_order.max_id#'>	--->
	</cfif>
