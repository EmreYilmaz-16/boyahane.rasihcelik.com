...
<cfset FORM.PROCESS_CAT =174>
<cfset attributes.PROCESS_CAT =174>
<cfset attributes.OLD_PROCESS_TYPE  =174>
<cfset FORM.SHIP_NUMBER  =attributes.parti_kodu>
<cfset attributes.SHIP_DATE  =attributes.date_1>

<cfset attributes.DELIVER_DATE =attributes.date_1>
<cfset attributes.DELIVER_DATE_FRM=attributes.date_1>
<cfset attributes.action_date =attributes.date_1>
<cfset attributes.BASKET_MONEY  ="TL">
<cfset attributes.BASKET_NET_TOTAL   =0>
<cfset attributes.department_id   =4>
<cfset attributes.location_id   =6>

<cfset attributes.kur_say   =3>
<cfset attributes.price    =0>
<cfset attributes.amount    =attributes.kilo_gram>
<cfset attributes.amount_other=attributes.metre>
<cfset attributes.unit     ="KG">
<cfset attributes.unit_other     ="M">
<cfset attributes.tax      =0>
<cfset attributes.row_lasttotal=0>
<cfset attributes.row_nettotal=0>
<cfset attributes.row_taxtotal=0>
...
<cfset attributes.txt_rate3_1   =1>
<cfset attributes.txt_rate3_2   =1>
<cfset attributes.txt_rate3_3   =1>
Attributeslar Burada <br>
<cfdump  var="#attributes#"><br>
Session Burada <br>
<cfdump  var="#session#">
<br>
<cfquery name="getPold" datasource="#dsn#">
	SELECT * FROM catalyst_prod.SETUP_PERIOD WHERE PERIOD_ID=#attributes.ACTIVE_PERIOD#
</cfquery>
<cfset New_DSN_2="#DSN#_#getPold.PERIOD_YEAR#_#session.ep.COMPANY_ID#">
<cfset New_Period_Id=attributes.ACTIVE_PERIOD>


Period Kontrolü Sanırım Burada
<!----<cfinclude template="check_our_period.cfm">---->
Period Kontrolü Bitmeli
<cfquery name="GET_NUMBER" datasource="#New_DSN_2#">
...
<cfquery name="GET_UNIQUE" datasource="#New_DSN_2#">
	SELECT 
		SHIP_NUMBER,
		PURCHASE_SALES,
		SHIP_ID
...
<cfdump  var="#attributes.deliver_date_frm#">**********<br>
<cfset attributes.DELIVER_DATE_H =hour(now())>
<cfset attributes.DELIVER_DATE_M =minute(now())>
...
<cfif GET_NUMBER.SHIP_TYPE eq 140><!--- servis giriş irsaliyesi ise seri nolar kontrol ediliyor--->
	<cfquery name="get_seriler" datasource="#dsn3#">
		...
		WHERE
			SERIAL_NO IN (SELECT SERIAL_NO FROM SERVICE_GUARANTY_NEW WHERE STOCK_ID = SG.STOCK_ID AND PERIOD_ID = #New_Period_Id# AND IS_SALE = 1) AND
			SERIAL_NO IN (SELECT SERIAL_NO FROM SERVICE_GUARANTY_NEW WHERE PROCESS_ID = #attributes.upd_id# AND PERIOD_ID = #New_Period_Id#)
	</cfquery>
...
<cflock name="#CreateUUID()#" timeout="60">
	<cftransaction>
    	<cfscript>
			add_relation_rows(
				action_type:'del',
				action_dsn : '#New_DSN_2#',
				to_table:'SHIP',
				to_action_id : attributes.UPD_ID
				);
		</cfscript>
		<cfquery name="UPD_PURCHASE" datasource="#New_DSN_2#">
			UPDATE
				SHIP
			SET
			...
			<cfelse>
				...
				SHIP_DETAIL = <cfqueryparam cfsqltype="cf_sql_varchar" value="#attributes.aciklama#">,
		...
		<cfquery name="DEL_SHIP_ROWS" datasource="#New_DSN_2#">
			DELETE FROM SHIP_ROW WHERE SHIP_ID = #attributes.UPD_ID#
		</cfquery>
		<cfset sr_row_id_list ="">
		<cfloop list="#attributes.rows_#" item="is">
		     <cfquery name="getUnitId" datasource="#New_DSN_2#">
            SELECT * FROM #dsn3#.STOCKS WHERE STOCK_ID=#attributes.stock_id#
        </cfquery>
        <cfset attributes.unit_id =getUnitId.PRODUCT_UNIT_ID>
        <cfset i="">
        <cfset "attributes.row_kontrol#i#"=true>
			...
			<cfquery name="ADD_SHIP_ROW" datasource="#New_DSN_2#" result="xx">
				INSERT INTO
					SHIP_ROW
				(
					NAME_PRODUCT,
		...
		<!--- Her halukarda belgeye göre seri update ediyoruz PY --->
			<cfquery name="upd_guaranty_old" datasource="#New_DSN_2#">
				UPDATE  
        ...
        <!--- eğer ürün silinirse girilmiş serilerde delete edilir PY 1114 --->
        	<cfquery name="get_stock_info" datasource="#New_DSN_2#">
	...
	<cfinclude  template="get_basket_money_js_.cfm"> 
		<cfscript>
			if(get_process_type.IS_STOCK_ACTION eq 1 and (not (isdefined("form.irsaliye_iptal") and form.irsaliye_iptal eq 1)))//Stok hareketi yapılsın
			{
				...
				{
					include('add_stock_rows.cfm');//sayfalar farklı modüllerden çağrıldıgı için buraya include edildi
					add_stock_rows(
			...
			if((get_process_type.process_type eq 74 or get_process_type.process_type eq 73) and isdefined("attributes.irsaliye_id_listesi") and len(attributes.irsaliye_id_listesi))//eğer iade irsaliyesi ise ilişkili mal alım iraliyelelerinin satın alma siparişleri bulunacak
			{
				...
				get_related_order_row = cfquery(datasource : "#New_DSN_2#", sqlstring : "SELECT  SRR.STOCK_ID, ORR.ORDER_ID,SR.SHIP_ID,SR.AMOUNT OLD_AMOUNT,SR.WRK_ROW_ID FROM #dsn3_alias#.ORDER_ROW ORR,SHIP_ROW SR,SHIP_ROW SRR WHERE ORR.WRK_ROW_ID = SR.WRK_ROW_RELATION_ID AND SR.WRK_ROW_ID = SRR.WRK_ROW_RELATION_ID AND SRR.SHIP_ID = #attributes.UPD_ID#");
				for(kk_order=1;kk_order<=get_related_order_row.recordcount;kk_order++)
				{
					get_total_ship_amount = cfquery(datasource : "#New_DSN_2#", sqlstring : "SELECT SUM(SRR.AMOUNT) AMOUNT FROM SHIP_ROW SRR WHERE SRR.WRK_ROW_RELATION_ID='#get_related_order_row.WRK_ROW_ID#'");
					...
					upd_row = cfquery(datasource : "#New_DSN_2#",is_select:0, sqlstring : "UPDATE #dsn3_alias#.ORDER_ROW_RESERVED SET STOCK_IN = #amount_new# WHERE ORDER_ID = #get_related_order_row.order_id[kk_order]# AND SHIP_ID = #get_related_order_row.ship_id[kk_order]# AND PERIOD_ID=#New_Period_Id# AND STOCK_ID = #get_related_order_row.STOCK_ID[kk_order]#");
					add_reserve_row(
		...
		<cfelse>
			...
			<cfif get_stock_fis.recordcount>
				...
				<cfquery name="del4" datasource="#New_DSN_2#">
					DELETE FROM STOCK_FIS_ROW WHERE FIS_ID = #get_stock_fis.FIS_ID#
				</cfquery>
				<cfquery name="del5" datasource="#New_DSN_2#">
					DELETE FROM STOCK_FIS WHERE FIS_ID = #get_stock_fis.FIS_ID#
				</cfquery>
				<cfquery name="del6" datasource="#New_DSN_2#">
					DELETE FROM STOCK_FIS_MONEY WHERE ACTION_ID = #get_stock_fis.FIS_ID#
				</cfquery>
				<cfquery name="del6" datasource="#New_DSN_2#">
					DELETE FROM #dsn3_alias#.INVENTORY_ROW WHERE PERIOD_ID = #New_Period_Id# AND ACTION_ID = #get_stock_fis.FIS_ID#
				</cfquery>
				<cfquery name="del6" datasource="#New_DSN_2#">
	...
	<!---	<cf_workcube_process_cat 
            ...
            <cf_add_log employee_id="#session.ep.userid#" log_type="0" action_id="#attributes.UPD_ID#" action_name="#SHIP_NUMBER# Güncellendi" paper_no="#SHIP_NUMBER#"  period_id="#New_Period_Id#" process_type="#get_process_type.PROCESS_TYPE#" data_source="#New_DSN_2#">--->
	</cftransaction>
</cflock>
<cfif isdefined("attributes.process_stage") and len(attributes.process_stage)>
	<cf_workcube_process 
		is_upd='1' 
		data_source='#New_DSN_2#' 
...
<cfinclude template="add_info_plus2.cfm">
<!---Ek Bilgiler--->
